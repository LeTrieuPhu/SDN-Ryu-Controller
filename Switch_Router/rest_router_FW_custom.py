...
#################################  chan ip #######################################################
    ...
    def register_router(cls, dp):
        ...
        if(dp.id==0x21):
                                ...
                                match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,icmpv4_type=icmp_type_Echo_Request, ipv4_src="100.0.0.1/8", ip_proto=inet.IPPROTO_ICMP)                            


                                inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions=[])]
        
                                mod = parser.OFPFlowMod(datapath=dp, priority=1000,
                                    match=match, 
                                    instructions=inst)

                                dp.send_msg(mod)
                                ############send drop for tcp 
                                match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,tcp_flags=0x02, ipv4_src="100.0.0.1/8", ip_proto=inet.IPPROTO_TCP)                            


                                inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions=[])]
        
                                mod = parser.OFPFlowMod(datapath=dp, priority=1001,
                                    match=match, 
                                    instructions=inst)

                                dp.send_msg(mod)                              

                                print("adding rules to dp.id", dp.id)
...
###########################  bat goi tin   ##########################################################
    def packet_in_handler(self, msg):
        ...
        if header_list:
            ...
            #in the Name of Allah, we startswith
            ...
            if pkt_arp:
                    ...
                    print("Ahmed, arp  packet from ", srcip ,"  to ",dstip," pkt_arp.opcode is ",pkt_arp.opcode)
                ...
                # if ICMP Protocol
                    ...
                    print("Ahmed, icmp  packet from ", srcip ,"  to ",dstip," ip_proto is ",inet.IPPROTO_ICMP)
                ...
                #  if TCP Protocol
                    ...
                    print("Ahmed, tcp  packet from ", srcip ,"  to ",dstip," src port is ",t.src_port," dst port is ",t.dst_port ,"flags are ",t.bits," ip_proto is", inet.IPPROTO_TCP)
